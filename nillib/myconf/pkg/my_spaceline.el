...
;; Keywords:
;; Created: 2019-04-27
;; Updated: 2025-04-18T00:14:06Z; # UTC
...
(use-package all-the-icons
  :config
  ;; If no font is installed, then install once. It looks 'all-the-icons-install-fonts is not called automatically as of 2022-11-28.
  (require 'cl-macs)
  (let* ((font-dest
    ...
    (if (and has-dest
             (not (file-exists-p (format "%s/all-the-icons.ttf" font-dest))))
        (progn
          (message "Install fonts for all-the-icons to %s" font-dest)
          (all-the-icons-install-fonts t))
      (warn "Failed to install font for all-the-icons: font-dest=%s, has-dest=%s %s" font-dest has-dest
            (format "%s/all-the-icons.ttf" font-dest)))))
