...
(use-package company-box
  ...
  :config
  (setq company-box-frame-parameters (append company-box-frame-parameters `((alpha . 100))))
  (setq company-box-backends-colors
        '(
          (company-capf . (:icon "yellow"
                                 :annotation (:background "white" :foreground "black")
                                 :selected (:background "yellow" :foreground "black")
                                 :candidate (:background "orange" :foreground "black")))
          (company-yasnippet . (:all "lime green"
                                     :selected (:background "lime green" :foreground "black")))
          )
        )
  )
