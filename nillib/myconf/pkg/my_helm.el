...
(use-package helm
  ;;  :after
  ;;  (tramp
  ;;  ;; Dependcies required by emacs27.2: tramp-archive-file-name-regexp in tramp-archive.el <- tramp <- helm-files
  ;;   )
  :config
;;;; 初期は無効にしておく iserch 中にM-m で toggle できる
  ...
  (use-package helm-gtags
    :after (helm)
    :config
    (unless (string< emacs-version "27")
      (require 'tramp-archive))
    (require 'helm-gtags) ; helm-gtags-mode-map
...
(global-set-key (kbd "M-E") 'helm-ls-git) ; List file in 'git ls-files'
