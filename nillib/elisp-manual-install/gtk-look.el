...
;; Version 20 - fix Glib::Param::Foo is GParamSpecFoo
;; Version 21 - class struct fallback GtkCellViewClass to GtkCellView etc
;; Version 22 - recognise "gtk-yes" as value of GTK_STOCK_YES etc
;; Version 23 - recognise Gtk2::Gdk::Cairo::Context->create as gdk_cairo_create
;; Version 24 - more Gtk2::Gdk::Cairo::Context
;; Version 25 - fix for crunching enum/struct/union names in .devhelp files
...
(defgroup gtk-lookup nil
  ...
  ;; pruning "gtk-lookup-" off gtk-lookup-devhelp-indices looks ok
  :prefix "gtk-lookup-"
...
LINK is separate from DIR to save a little memory as the (DIR
TYPE) tail is common to all the links in one manual.
...
" nil t)
                 (let ((type (match-string 2))
                   ...
                   (when (string-match "\\`\\(enum\\|struct\\|union\\) "
                                       name)
                     (setq type (match-string 1 name)) ;; before changing name
...
`gtk-lookup-symbol-interactive-arg'."
  ...
  (when str
    (let* ((case-fold-search nil)
           ...
           ;; gtk-go-forward -> GTK_STOCK_GO_FORWARD, the string value of
           ;; those stock symbols
           ;; digits for "gtk-zoom-100" GTK_STOCK_ZOOM_100
           (stock (and (string-match "\\`\\([a-zA-Z]+\\)-[a-zA-Z0-9-]+\\'" str)
                       (let ((up
                              (upcase (concat (match-string 1 str)
                                              "_STOCK"
                                              (substring str (match-end 1))))))
                         (while (string-match "-" up)
                           (setq up (replace-match "_" t t up)))
                         ;; `up' string like "GTK_STOCK_YES"
                         (and (assoc up (gtk-lookup-cache-init))
                              up)))))
      ...
      ;; gtk2-perl "Gtk2::Gdk::Cairo::Context" -> gdk_cairo for
      ;; "Gtk2::Gdk::Cairo::Context->create" is gdk_cairo_create or
      ;; "Gtk2::Gdk::Cairo::Context::set_source_pixbuf" is
      ;; gdk_cairo_set_source_pixbuf
      ;; there's no actual Gdk Cairo class though, Gtk2::Gdk::Cairo::Context
      ;; is only a perl-level subclass
      (if (string-match "\\`Gtk2::Gdk::Cairo::Context\\(->\\|::\\)" str)
          (setq str (replace-match "gdk_cairo_" t nil str)))
      ...
      ;; gtk2-perl "Glib::Param::Double" -> "GParamSpecDouble" etc, an
      ;; inconsistency in the class naming
      (if (string-match "\\`Glib::\\(Param\\)::" str)
          (setq str (replace-match "GParamSpec" t nil str)))
      ...
      (if (string-match "[_-]" str)
          ...
          (progn
            ...
            ;; gtk-yes -> GTK_STOCK_YES, if there's no other function,
            ;; variable, macro etc reached by `str' here
            ;; as of Gtk 2.20 none of the GTK_STOCK_FOO also exists as GTK_FOO
            (and stock
                 (not (gtk-lookup-assoc-string-ignore-case
                       str (gtk-lookup-cache-init)))
                 (setq str stock)))


        ;; one word class name, no - or _ chars

        ;; gtk2-perl "Gtk::Object" -> "GtkObject" separators
        ;; including double-colon bareword like "Gtk::Label::" -> "GtkLabel",
        (while (string-match "::" str)
          (setq str (replace-match "" t t str)))

        ;; GtkCellViewClass -> GtkCellView, if there's nothing for the
        ;; "Class" form.  The Class struct is generally only in the docs if
        ;; there's class funcs or whatnot a subclass might act on,
        ;; eg. GtkWidgetClass.  Falling back to the instance struct at least
        ;; gets to the docs of the relevant widget etc if not to anything
        ;; specific about the class struct.
        (and (string-match "Class\\'" str)
             (not (assoc str (gtk-lookup-cache-init)))
             (assoc (substring str 0 -5) gtk-lookup-cache)
...
`browse-url-browser-function' to choose the viewer and with
...
;; LocalWords: devhelp libgtk docstring datatype gz usr unparent canonicalized func keyval TreeStore GtkWidget Pango regexps
